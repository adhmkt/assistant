...
async def message(sid, data):
    ...
    thread_id = database_manager.get_thread_id(sid)
    session_id = database_manager.get_session_id(sid)
    if thread_id:
        assistant_id = database_manager.get_assistant_id(sid)
        
        await send_bot_response(thread_id, data, sid, assistant_id, session_id)
    else:
...
async def send_bot_response(thread_id, message, sid , assistant_id, session_id):
   
    database_manager = DatabaseManager
    assistant_id = database_manager.get_assistant_id(sid)
    # assistant_id = session_manager.get_assistant_id(sid)  # Make sure to call the thod with sid
    user_id = database_manager.get_user_id(sid)
    ...
    if assistant_id:
        ...
        response = await get_bot_response(thread_id, user_id, message, assistant_id, client, session_id)
